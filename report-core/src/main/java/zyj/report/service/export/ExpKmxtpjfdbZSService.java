...
@Service
public class ExpKmxtpjfdbZSService extends BaseRptService {
...
//		Map<String,Map<String,Object>> clsNameMap = getClassCache();
		...
		ExportUtil.createExpExcel(new String[][]{titleList1,titleList2}, conList, pathFile+"科目小题平均分对比.xls",avgmerge,parmter.get("subjectName")+"科目小题平均分对比");
	}
	...
	private List<List<Object>> getQuestionDetail(List<Map<String,Object>>question,List<Map<String,Object>>sub,Map<String,Object>subAll,String[] itemList,String KEY,String name,Map<String,Object>util){
		...
		for(Map<String,Object> m : question){
			...
			if(!k.contains(id)){
				...
				double obj_avg = Double.parseDouble(subtrans.get(id).get("OBJ_AVG_SCORE")==null?"0":subtrans.get(id).get("OBJ_AVG_SCORE").toString());
				double sub_avg = Double.parseDouble(subtrans.get(id).get("SUB_AVG_SCORE")==null?"0":subtrans.get(id).get("SUB_AVG_SCORE").toString());
				double all_avg = Double.parseDouble(subtrans.get(id).get("AVG_SCORE").toString());
				resdatalist.add(obj_total==0?"":obj_total);
				resdatalist.add(obj_avg==0?"":obj_avg);
				resdatalist.add(sub_total==0?"":sub_total);
				resdatalist.add(sub_avg==0?"":sub_avg);
				resdatalist.add(fullscore);
				resdatalist.add(all_avg);
