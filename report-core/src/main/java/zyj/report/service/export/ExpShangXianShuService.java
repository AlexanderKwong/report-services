...
@Service
public class ExpShangXianShuService extends BaseRptService {
...
// List<Map<String,Object>> subjects_cur = getSubjectCache();
		...
		double[] scoreLine = CalToolUtil.getSubjectScoreLine(subject0);
		if(scoreLine != null)
		...
		for(SubjectInfo s : subjectList){
			...
			zyj.report.common.ExportUtil.createExpExcel(rows, beanList, pathFile + "上线数" + ((("WK".equals(subject) || "LK".equals(subject)) ? "_" + subjects.get(subject).substring(0, 2) + ".xls" : ".xls")), mgArray);
	}
	/**
	 * 
